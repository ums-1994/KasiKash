...
from rewards import rewards_bp  # Import the rewards blueprint
...
def home():
    ...
    try:
        ...
        # Add recent_activities for dashboard timeline and chart
        recent_activities = []
        try:
            with support.db_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    for row in cur.fetchall():
                        recent_activities.append({
                            ...
                            'status': 'Processed' if row[0] == 'contribution' else 'Completed'
                        })
        except Exception as e:
            logger.warning(f"Could not fetch recent activities: {e}")
            # Leave recent_activities as empty list if error
        ...
        return render_template('dashboard.html',
                            ...
                            recent_activities=recent_activities) # Pass recent activities to template
    except Exception as e:
                ...
                for pm_tuple in payment_methods_tuples:
                    ...
                    try:
                        ...
                        if pm_dict['type'] in ['credit_card', 'debit_card', 'card']:
                            ...
                            masked_details = ', '.join(  
                                f"{k}: {v}" for k, v in details_json.items() if 'number' not in k and 'card' not in k 
                            )
                    except Exception as e:
...
def profile():
    try:
        ...
        with support.db_connection() as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                # Fetch all user info fields
                cur.execute("""
                    SELECT username, email, full_name, phone, id_number, date_of_birth, address, bio, profile_picture, kyc_status, last_login
                    FROM users
                    WHERE firebase_uid = %s
                """, (session['user_id'],))
                user_info = cur.fetchone()
                if not user_info:
                    ...
                    return redirect(url_for('home'))
                # Set defaults for all fields if None
                for key in ['username', 'email', 'full_name', 'phone', 'id_number', 'date_of_birth', 'address', 'bio', 'profile_picture', 'kyc_status', 'last_login']:
                    user_info[key] = user_info.get(key) or ''
