...
app.config['MAIL_DEFAULT_SENDER'] = (
    'KasiKash App', os.getenv('MAIL_USERNAME'))
mail = Mail(app)
...
def analysis():
    ...
    df['Amount'], errors='coerce').fillna(0)
            ...
            try:
                ...
                pie = support.meraPie(df=type_summary, names='Type', values='Amount', hole=0.7,
                                    ...
                                    margin=dict(t=1, b=1, l=1))

                # Bar chart by description
                desc_summary = df.groupby(['Description', 'Type'])[
    ...
    df, 'Date', 'Amount', 'Type', 'Amount', slider=False)
...
def login_validation():
    ...
    user_record.display_name or email)  # Ensure it's a string
                ...
                session['profile_picture'] = getattr(
    ...
    f"Firebase UID mismatch or not set. Updating references for user {email}. Old: {old_firebase_uid}, New: {new_firebase_uid}")
                                    ...
                                    update_queries = [
                                        ...
                                        ("UPDATE stokvels SET created_by = %s WHERE created_by = %s",
                                         (new_firebase_uid, old_firebase_uid)),
                                        ("UPDATE stokvel_members SET user_id = %s WHERE user_id = %s",
                                         (new_firebase_uid, old_firebase_uid)),
                                        ("UPDATE transactions SET user_id = %s WHERE user_id = %s",
                                         (internal_user_id, internal_user_id)),
                                        ("UPDATE chat_history SET user_id = %s WHERE user_id = %s",
    ...
    f"Executing update query for related table: {query} with params {params}")
                                            ...
                                            print(
                                                f"Error updating related table {query}: {str(update_e)}")
...
def send_password_reset_email(to_email, reset_link):
    ...
    try:
        ...
        print(
            f"SendGrid password reset email sent status code: {response.status_code}")
        ...
        if response.status_code == 202:
            ...
            print(
                f"Failed to send password reset email. Status: {response.status_code}")
    ...
    if stokvel_id:
        ...
        support.execute_query(
            ...
            (stokvel_id,
             user_id,
             'admin'))
        # Add the creator as the first member or update their role to admin if
        # already a member
        existing = support.execute_query(
    "search",
    "SELECT id FROM stokvel_members WHERE stokvel_id = %s AND user_id = %s",
    (stokvel_id,
     user_id))
        if existing:
            support.execute_query(
    "update",
    "UPDATE stokvel_members SET role = %s WHERE stokvel_id = %s AND user_id = %s",
    ('admin',
    stokvel_id,
     user_id))
        else:
    ...
    user_id,
     'admin'))
        ...
        with support.db_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
    "UPDATE users SET role = 'admin' WHERE firebase_uid = %s", (user_id,))
                conn.commit()
        session['role'] = 'admin'
...
def make_contribution():
    if request.method == 'POST':
        ...
        try:
            ...
            flash(
                f"Contribution of R{amount:.2f} recorded successfully{payment_info}! You earned {int(amount)} reward points!")
            return redirect(url_for('contributions'))
        except ValueError:
...
def contribute_to_goal():
    ...
    (firebase_uid,
        ...
        flash(
            f"Successfully contributed R{amount:.2f} to your savings goal{payment_info}!")
        return redirect('/savings_goals')
    ...
    try:
        with support.db_connection() as conn:
            with conn.cursor() as cur:
                ...
                if not column:
                    print(
                        f"Debug: Invalid setting or section for update: {section}, {setting}")
                    return False
...
def profile():
    try:
        with support.db_connection() as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                # Get user data, ensuring all columns including kyc_status are
                # selected
                cur.execute("""
                    SELECT u.*,
                ...
                try:
                    ...
                    print(
                        f"Error fetching Firebase user for verification status: {e}")
                    is_verified = False
...
def send_email(to_email, subject, body):
    try:
        ...
        print(
            f"DEBUG: send_email - SMTP_PASSWORD: {'*' * len(str(smtp_password))}")
...
def pricing():
    try:
        ...
        return render_template('pricing.html', pricing_plans=[])
...
def download_stokvel_statement_pdf(stokvel_id):
    ...
    Line(
        ...
        540,
        0,
        strokeColor=colors.HexColor('#22d3ee'),
         strokeWidth=3))
...
def handle_join_stokvel_room(data):
    ...
    if stokvel_id and user_id:
        ...
        emit('status',
    {'msg': f'User joined stokvel chat.'},
...
def handle_send_message(data):
    ...
    with support.db_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
    "INSERT INTO stokvel_chat_messages (stokvel_id, user_id, message) VALUES (%s, %s, %s) RETURNING timestamp",
    (stokvel_id,
    user_id,
     message))
...
@app.route('/stokvel/<int:stokvel_id>/remove_member/<int:member_id>',
           methods=['POST'])
@login_required
    ...
    if request.method == 'POST':
        ...
        if email and inviter_name and stokvel_name:
            try:
                ...
                flash(
    'Failed to send referral email. Please try again later.',
     'danger')
        else:
    ...
    try:
        with support.db_connection() as conn:
            with conn.cursor() as cur:
                    ...
                    if not loan:
                        flash(
    'Loan not found or not eligible for repayment.', 'danger')
                        return redirect(url_for('pay_back_loan'))
                    ...
                    if repayment_amount <= 0:
                        flash(
    'Repayment amount must be greater than 0.', 'danger')
                        return redirect(url_for('pay_back_loan'))
                    ...
                    flash(
    f'Loan repayment of R{repayment_amount:.2f} recorded successfully!',
     'success')
                    return redirect(url_for('pay_back_loan'))
        ...
        try:
            ...
            with support.db_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    if not cur.fetchone():
                        flash(
    'You are not a member of this stokvel.', 'danger')
                        return redirect(url_for('request_loan'))

                    # Check if user already has a pending loan from this
                    # stokvel
                    cur.execute("""
...
def handle_fetch_messages(data):
    ...
    with support.db_connection() as conn:      
        with conn.cursor() as cur:
            ...
            """, (stokvel_id,))
                rows = cur.fetchall()
                print(f"DEBUG: Fetched {len(rows)} messages for stokvel {stokvel_id}")
                if rows:
                   for row in rows:
                    messages.append({
                        'user_id': row[0],
