document.addEventListener('DOMContentLoaded', () => {
    ...
    const chatbotClose = document.getElementById('chatbot-close') || document.getElementById('chatbot-toggle'); // The close (X) or toggle button
    ...
    const appModeLabel = document.getElementById('app-mode-label') || document.getElementById('current-mode');
    ...
    function updateModeDisplay() {
        const switchIcon = document.getElementById('switch-icon') || document.getElementById('current-mode');
        const modeLabel = document.getElementById('mode-label') || document.querySelector('.mode-label');
        if (isAIMode) {
            ...
            if (modeLabel) {
                ...
                modeLabel.classList.remove('text-orange-400', 'text-green-400');
                modeLabel.classList.add('text-cyan-200');
            }
            if (currentMode) currentMode.textContent = '🤖 AI Mode';
        } else {
            ...
            if (currentMode) currentMode.textContent = '📱 App Mode';
        }
    }
    ...
    // Close/minimize chat window from close (X) or toggle button
    if (chatbotClose) {
